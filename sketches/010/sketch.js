...
function draw() {
  ...
  let dataShaped = data.filter(point => abs(dist(width / 2, height / 2,
  ...
  for (let i = 0; i < dataShaped.length; i++) {
    ...
    restOfTheData.sort(function(a, b) {
      return abs(dist(a.x, a.y, firstPoint.x, firstPoint.y)) - 
    ...
    if(abs(dist(firstPoint.x, firstPoint.y, secondPoint.x, secondPoint.y)) > minDist) {
      
       strokeWeight(map(dist(secondPoint.x, secondPoint.y,
                  width/2, height/2), 0, radius, size/2, size/8));
  ...
  for (let i = 0; i < dataShaped.length; i++) {
    ...
    circle(dataShaped[i].x, dataShaped[i].y, 
          map(dist(dataShaped[i].x, dataShaped[i].y,
                  width/2, height/2), 0, radius, size, size/4));
...
function makeData(length, startWidth, endWidth, startHeight, endHeight) {
  ...
  for (i = 0; i < length; i++) {
    ...
    array.push(createVector(random(startWidth, endWidth),
      random(startHeight, endHeight)));
